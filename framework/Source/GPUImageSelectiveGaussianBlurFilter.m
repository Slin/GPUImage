...
@implementation GPUImageSelectiveGaussianBlurFilter
{
	GLint verticalPassFocusModeUniform;
	GLint verticalPassFocusPositionUniform;
	GLint verticalPassFocusRadiusUniform;
	GLint verticalPassFocusFallOffRateUniform;
	GLint verticalPassPixelScaleUniform;
	
	GLint horizontalPassFocusModeUniform;
	GLint horizontalPassFocusPositionUniform;
	GLint horizontalPassFocusRadiusUniform;
	GLint horizontalPassFocusFallOffRateUniform;
	GLint horizontalPassPixelScaleUniform;
	}
...
+ (NSString *)fragmentShaderForStandardBlurOfRadius:(NSUInteger)blurRadius sigma:(CGFloat)sigma;
{
    ...
    [shaderString appendFormat:@"\
	 ...
	 if(focusMode == 2)\n\
	 {\n\
	 blurIntensity = 1.0 - smoothstep(focusPosition.y - focusRadius - focusFallOffRate, focusPosition.y - focusRadius, texCoordinates.y);\n\
	 blurIntensity += smoothstep(focusPosition.y + focusRadius, focusPosition.y + focusRadius + focusFallOffRate, texCoordinates.y);\n\
	 }\n\
	 else if(focusMode == 1)\n\
	 {\n\
	 mediump vec2 diff = texCoordinates - focusPosition;\n\
	 lowp float dist = length(diff*pixelScale);\n\
	 blurIntensity = smoothstep(focusRadius, focusRadius + focusFallOffRate, dist);\n\
	 }\n\
    ...
    for (NSUInteger currentBlurCoordinateIndex = 0; currentBlurCoordinateIndex < (blurRadius * 2 + 1); currentBlurCoordinateIndex++)
    {
        ...
        if (offsetFromCenter < 0)
        {
            [shaderString appendFormat:@"sum += texture2D(inputImageTexture, blurCoordinates[%lu]) * blurIntensity * %f;\n", (unsigned long)currentBlurCoordinateIndex, standardGaussianWeights[-offsetFromCenter]];
        }
        else
        {
            [shaderString appendFormat:@"sum += texture2D(inputImageTexture, blurCoordinates[%lu]) * blurIntensity * %f;\n", (unsigned long)currentBlurCoordinateIndex, standardGaussianWeights[offsetFromCenter]];
...
+ (NSString *)vertexShaderForOptimizedBlurOfRadius:(NSUInteger)blurRadius sigma:(CGFloat)sigma;
{
    ...
    // Inner offset loop
    [shaderString appendString:@"blurCoordinates[0] = vec2(0.0, 0.0);\ntexCoordinates = inputTextureCoordinate.xy;"];
    for (NSUInteger currentOptimizedOffset = 0; currentOptimizedOffset < numberOfOptimizedOffsets; currentOptimizedOffset++)
    {
        [shaderString appendFormat:@"\
         blurCoordinates[%lu] = singleStepOffset * %f;\n\
...
+ (NSString *)fragmentShaderForOptimizedBlurOfRadius:(NSUInteger)blurRadius sigma:(CGFloat)sigma;
{
    ...
    // Inner texture loop
    [shaderString appendFormat:@"sum += texture2D(inputImageTexture, texCoordinates+blurCoordinates[0]*blurIntensity) * %f;\n", standardGaussianWeights[0]];
    ...
    for (NSUInteger currentBlurCoordinateIndex = 0; currentBlurCoordinateIndex < numberOfOptimizedOffsets; currentBlurCoordinateIndex++)
    {
        ...
        [shaderString appendFormat:@"sum += texture2D(inputImageTexture, texCoordinates+blurCoordinates[%lu]*blurIntensity) * %f;\n", (unsigned long)((currentBlurCoordinateIndex * 2) + 1), optimizedWeight];
    ...
    if (trueNumberOfOptimizedOffsets > numberOfOptimizedOffsets)
    {
        ...
        for (NSUInteger currentOverlowTextureRead = numberOfOptimizedOffsets; currentOverlowTextureRead < trueNumberOfOptimizedOffsets; currentOverlowTextureRead++)
        {
            ...
            [shaderString appendFormat:@"sum += texture2D(inputImageTexture, texCoordinates+blurCoordinates[0] + singleStepOffset * blurIntensity * %f) * %f;\n", optimizedOffset, optimizedWeight];
            [shaderString appendFormat:@"sum += texture2D(inputImageTexture, texCoordinates+blurCoordinates[0] - singleStepOffset * blurIntensity * %f) * %f;\n", optimizedOffset, optimizedWeight];
        }
    }
...
- (void)switchToVertexShader:(NSString *)newVertexShader fragmentShader:(NSString *)newFragmentShader;
{
    runSynchronouslyOnVideoProcessingQueue(^{
		...
		verticalPassFocusModeUniform = [filterProgram uniformIndex:@"focusMode"];
		verticalPassFocusPositionUniform = [filterProgram uniformIndex:@"focusPosition"];
		verticalPassFocusRadiusUniform = [filterProgram uniformIndex:@"focusRadius"];
		verticalPassFocusFallOffRateUniform = [filterProgram uniformIndex:@"focusFallOffRate"];
		verticalPassPixelScaleUniform = [filterProgram uniformIndex:@"pixelScale"];
		...
		horizontalPassFocusModeUniform = [filterProgram uniformIndex:@"focusMode"];
		horizontalPassFocusPositionUniform = [filterProgram uniformIndex:@"focusPosition"];
		horizontalPassFocusRadiusUniform = [filterProgram uniformIndex:@"focusRadius"];
		horizontalPassFocusFallOffRateUniform = [filterProgram uniformIndex:@"focusFallOffRate"];
		horizontalPassPixelScaleUniform = [filterProgram uniformIndex:@"pixelScale"];
...
- (void)setUniformsForProgramAtIndex:(NSUInteger)programIndex;
{
	...
	if(programIndex == 0)
	{
		glUniform1i(verticalPassFocusModeUniform, _focusMode);
		glUniform1f(verticalPassFocusFallOffRateUniform, _focusFallOffRate);
		glUniform2f(verticalPassFocusPositionUniform, _focusPosition.x, _focusPosition.y);
		glUniform1f(verticalPassFocusRadiusUniform, _focusRadius);
		glUniform2f(verticalPassPixelScaleUniform, _pixelScale.x, _pixelScale.y);
	}
	else
	{
		glUniform1i(horizontalPassFocusModeUniform, _focusMode);
		glUniform1f(horizontalPassFocusFallOffRateUniform, _focusFallOffRate);
		glUniform2f(verticalPassFocusPositionUniform, _focusPosition.x, _focusPosition.y);
		glUniform1f(verticalPassFocusRadiusUniform, _focusRadius);
		glUniform2f(verticalPassPixelScaleUniform, _pixelScale.x, _pixelScale.y);
	}
...
- (void)setFocusPosition:(CGPoint)focusPosition
{
...
- (void)setFocusFallOffRate:(float)focusFallOffRate
{
	_focusFallOffRate = focusFallOffRate;
}

- (void)setPixelScale:(CGPoint)pixelScale
{
